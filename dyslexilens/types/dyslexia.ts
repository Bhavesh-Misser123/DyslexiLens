...
/** Analysis data for any word that can be selected for help */
export interface WordAnalysis {
  word: string; // The word as it appears
...
export interface DyslexiaResult {
  ...
  hard_words: HardWord[]; // Up to 10 difficult words (legacy - keep for compatibility)
  word_analysis?: WordAnalysis[]; // Syllable/pronunciation data for all significant words (optional)
  reading_tips: string[]; // 2-4 actionable tips for reading this specific text
}

/** Processing state used across UI components */
export type ProcessingStatus = "idle" | "ocr" | "gemini" | "done" | "error";
